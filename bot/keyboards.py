...
def result_keyboard(
    ...
    buttons = []
    if playable_url:
        buttons.append([InlineKeyboardButton("▶️  Play Online", url=playable_url)])
    if download_url:
        buttons.append([InlineKeyboardButton("⬇️  Download", url=download_url)])
    if quality_options:
        buttons.append([InlineKeyboardButton("🎞  Change Quality", callback_data="quality:menu")])
    if original_url and len(original_url) <= 256:
        buttons.append([
            InlineKeyboardButton("📋  Copy Link", copy_text=CopyTextButton(text=original_url))
        ])
    buttons.append([
...
def quality_keyboard(qualities: tuple[str, ...]) -> InlineKeyboardMarkup:
    ...
    for quality in ordered:
        current.append(InlineKeyboardButton(f"🎞 {quality}", callback_data=f"quality:{quality}"))
        if len(current) == 2:
            rows.append(current)
            current = []
    if current:
        rows.append(current)
    rows.append([InlineKeyboardButton("↩️ Back", callback_data="quality:back")])
    return InlineKeyboardMarkup(rows)


def file_selection_keyboard(files: tuple, max_files: int = 25) -> InlineKeyboardMarkup:
    rows = []
    for index, file_result in enumerate(files[:max_files]):
        ...
        if len(name) > 36:
            name = name[:33] + "..."
        rows.append([
            InlineKeyboardButton(f"{index + 1}. {name}", callback_data=f"file:{index}")
        ])
    if len(files) > max_files:
        rows.append([InlineKeyboardButton(
            f"ℹ️ Showing first {max_files} files", callback_data="noop"
        )])
    rows.append([InlineKeyboardButton("↩️ Back", callback_data="files:back")])
    return InlineKeyboardMarkup(rows)
